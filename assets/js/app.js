...
class LotofacilEstrategica {
    ...
    init() {
        ...
        // Verificar se os campos estão vazios e forçar atualização
        setTimeout(() => {
    ...
    verificarCamposVazios() {
        ...
        if (!concurso || !dezenas) {
            console.log('🔍 Campos vazios detectados - forçando nova busca...');
    ...
    iniciarAtualizacaoAutomatica() {
        ...
        console.log('⏰ Frequência: A cada 1 minuto + ao ficar visível');
        console.log('🎯 Campos atualizados: Concurso, Data e Dezenas automaticamente');
        ...
        setTimeout(() => {
            this.buscarUltimoResultadoAutomatico();
        }, 100); // Buscar mais rápido
        
        // Buscar também quando a página fica visível novamente
        if (typeof document !== 'undefined') {
            document.addEventListener('visibilitychange', () => {
                if (!document.hidden) {
                    console.log('👁️ Página ficou visível - verificando novos resultados...');
                    this.buscarUltimoResultadoAutomaticoSilencioso();
                }
            });
        }
        
        // Configurar atualização automática a cada 1 minuto (mais frequente)
        setInterval(() => {
        ...
        if (indicadorAuto) {
            indicadorAuto.title = 'Atualização automática ativa (a cada 1 minuto)';
        }
    }
    ...
    tentarBuscarResultadoAutomatico() {
        ...
        if (btnAtualizar) {
            ...
            btnAtualizar.innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i>Buscando...';
    ...
    async buscarResultadoPorData(dataAposta) {
        // Removido o bloqueio por data - agora sempre retorna o último resultado disponível para conferência
    ...
    conferirAposta(id) {
        ...
        // Removido o bloqueio - agora o usuário pode conferir quando quiser
